...
function resolveGrowthSpeed(grassGrowthSpeed, growthScore) {
  if (grassGrowthSpeed && grassGrowthSpeed !== 'auto') return grassGrowthSpeed;
  const s = growthScore ?? 50;
  if (s < 35) return 'slow';
  if (s > 65) return 'fast';
  return 'medium';
}
...
function calculateLawnOptimization({
  ...
  // User-supplied heights always take priority; profile heights are the fallback.
  const baseTargetMm = targetGrassHeightMm ?? profile.targetHeightMm;
  const baseMinMm    = minHeightMm         ?? profile.minHeightMm;
  const baseMaxMm    = maxHeightMm         ?? profile.maxHeightMm;
  ...
  } else if (shadeLevel === 'partial_shade') {
    recommendedHeightMm += 4;
    reasons.push(__ ? __('services.profile.reasons.shade') : 'Maintain higher grass height for shade');
  }
  ...
  const resolvedGrowthSpeed = resolveGrowthSpeed(grassGrowthSpeed, growthScore);

  if (mowingFrequencyStrategy === 'adaptive') {
    // Growth speed adjustment (auto-derived from growth score unless user-overridden)
    const growthMult = GROWTH_SPEED_FREQ_MULT[resolvedGrowthSpeed] ?? 1.0;
  ...
  const recommendations = _buildRecommendations({
    ...
    growthScore,
    resolvedSeason,
    grassGrowthSpeed: resolvedGrowthSpeed,
    __,
  });
