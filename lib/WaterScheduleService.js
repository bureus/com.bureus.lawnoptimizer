  ...
  calculate({
    ...
    if (waterDeficitMm === 0) {
      return this._result({
        ...
        recommended: false, due: false, nextDate: null, amount: 0,
        status: 'No watering needed — rainfall target reached',
    ...
    if (forecastRainNext7DaysMm >= waterDeficitMm) {
      return this._result({
        rainThisWeekMm, irrigationThisWeekMm, totalWaterThisWeekMm, waterDeficitMm,
        recommended: false, due: false, nextDate: null, amount: 0,
        status: 'No watering needed — enough rain expected this week',
    ...
    if (forecastRainNext24hMm >= waterDeficitMm) {
      return this._result({
        rainThisWeekMm, irrigationThisWeekMm, totalWaterThisWeekMm, waterDeficitMm,
        recommended: false, due: false, nextDate: null, amount: 0,
        status: 'Delay watering — rain expected tomorrow',
    ...
    if (suggestedAmount < 3) {
      return this._result({
        ...
        status: `Water deficit ${waterDeficitMm} mm — below minimum actionable amount`,
    ...
    let status;
    if (isHeatStress) {
      status = `Watering recommended: ${suggestedAmount} mm — heat stress, water lightly and repeat`;
    } else if (nextWateringDate) {
      const d       = parseIsoDate(nextWateringDate);
      const dayName = d ? DAY_NAMES[d.getUTCDay()] : nextWateringDate;
      status = `Watering window: ${dayName} ${timeStr}`;
    } else {
      status = `Watering recommended: ${suggestedAmount} mm needed this week`;
    }
