...
class LawnScoringService {
  ...
  assess(temps, __ = null) {
    ...
    const statusText           = this._statusText(rootZone, growthScore, frostRisk, heatStressRisk, __);
  ...
  _statusText(rootZone, growthScore, frostRisk, heatStressRisk, __ = null) {
    if (frostRisk)           return __ ? __('services.scoring.frost_risk')        : 'Frost risk – protect your lawn';
    if (heatStressRisk)      return __ ? __('services.scoring.heat_stress')       : 'Heat stress – consider watering';
    if (rootZone < 5)        return __ ? __('services.scoring.dormant')           : 'Dormant – grass is not growing';
    if (rootZone < 8)        return __ ? __('services.scoring.very_slow_growth')  : 'Very slow growth';
    if (rootZone < 12)       return __ ? __('services.scoring.early_growth')      : 'Early growth – lawn is waking up';
    if (growthScore >= 70)   return __ ? __('services.scoring.strong_growth')     : 'Strong growth conditions';
    if (growthScore >= 45)   return __ ? __('services.scoring.good_growth')       : 'Good growth conditions';
    return __ ? __('services.scoring.moderate_growth') : 'Moderate growth conditions';
  }
