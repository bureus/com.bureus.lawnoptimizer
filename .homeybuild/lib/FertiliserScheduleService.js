  ...
  calculate({
    ...
    if (!lastDate) {
      return {
        ...
        daysRemaining: null,
        due:           false,
        status:        'Set last fertiliser date to enable scheduling.',
        reason:        'no_date',
      };
    }
    ...
    if (!isMonthInSeason(currentMonth, seasonStartMonth, seasonEndMonth)) {
      return {
        ...
        daysRemaining,
        due:   false,
        status: 'Outside fertiliser season',
    ...
    if (effectiveTemp < minSoilTemp) {
      return {
        ...
        due:   false,
        status: overdue
          ? `Due, but delayed — soil temperature below ${minSoilTemp} °C`
          : `Too early — soil temperature below ${minSoilTemp} °C`,
    ...
    if ((growthScore ?? 50) < 35) {
      return {
        ...
        due:   false,
        status: overdue
          ? 'Due, but delayed — lawn not growing actively enough'
          : 'Too early — lawn not growing actively enough',
        reason: 'low_growth',
      };
    }
    ...
    if (grassType === 'warm_season' && effectiveTemp < 14) {
      return {
        ...
        due:   false,
        status: overdue
          ? 'Due, but delayed — soil too cool for warm-season grass'
          : 'Waiting — soil too cool for warm-season grass',
    ...
    if (rain48h > rainWindowMax) {
      return {
        ...
        due:   false,
        status: overdue
          ? 'Due, but delayed — heavy rain expected'
          : 'Delay — heavy rain expected',
        reason: 'heavy_rain',
      };
    }
    ...
    if (due) {
      if (rain48h >= rainWindowMin && rain48h <= rainWindowMax) {
        status = 'Due now — light rain expected, ideal timing';
      } else if (rain48h === 0) {
        status = 'Due now — no rain expected, water after applying';
      } else {
        status = 'Due now — conditions look good';
      }
    } else if (daysRemaining === 1) {
      status = 'Due tomorrow';
    } else if (daysRemaining <= 7) {
      status = `Recommended in ${daysRemaining} days`;
    } else if (daysRemaining <= 14) {
      status = `Next fertiliser in ${daysRemaining} days`;
    } else {
      status = `Next fertiliser date: ${nextDateStr}`;
    }
